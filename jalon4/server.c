...
void filesend(struct message msgstruct, char * buff, struct  pollfd pollfds, LinkList * head,LinkList * nodeclient)
{
    					int rec2;
						rec2 = recv(pollfds.fd, buff, msgstruct.pld_len, 0);
						if(rec2 == -1){
                        ...
                        printf("recv demande send\n");
						LinkList *Nodefind;
						int flags = IsNameUsed(head,msgstruct.infos);
						if(flags ==1){
							Nodefind = Find_Node(head,msgstruct.infos);
                            //char transmet[MSG_LEN];
                            //sprintf(transmet,"User: [%s] want to send you a file named [%s], accept?[Y/N]\n",nodeclient->nickname,buff);
                            //strcat(buff,"\n");
                            msgstruct.type = FILE_REQUEST;
							strcpy(msgstruct.infos,nodeclient->nickname);
							msgstruct.pld_len = strlen(buff);
                            //Nodefind->fileflagsrecv = 1; 
                            // 如果 = 0 =>  Y N 都直接传递为echosend
                            // 如果 = 1 =》 Y N 函数
                            strcpy(Nodefind->filename,buff);
							if (send(Nodefind->fd, &msgstruct, sizeof(msgstruct), 0) <= 0) {
								printf("send1 error\n");
								perror("send erreur");
								exit(EXIT_FAILURE);
								}
							if (send(Nodefind->fd,buff, msgstruct.pld_len, 0) <= 0) {
								printf("send nick list erreur\n");
								perror("send errer");
								exit(EXIT_FAILURE);
								}
...
void fileaccept(struct message msgstruct, char * buff, struct  pollfd pollfds, LinkList * head,LinkList * nodeclient)
{
						LinkList *Nodefind;
						int rec2;
						rec2 = recv(pollfds.fd, buff, msgstruct.pld_len, 0);
						...
						if(flags ==1){
                            ...
                            sprintf(buff,"%s:%d",CLIENT_COM_ADDR,CLIENT_COM_PORT);
                            //strcat(buff,"\n");
							...
							memset(buff,'\0',MSG_LEN);
							strcpy(buff,"[SERVER]: name dont exist or offline,sorry\n");
							msgstruct.pld_len = strlen(buff);
							msgstruct.type = SERVER;
							if (send(pollfds.fd, &msgstruct, sizeof(msgstruct), 0) <= 0) {
								printf("send1 error\n");
...
void filereject(struct message msgstruct, char * buff, struct  pollfd pollfds, LinkList * head,LinkList * nodeclient)
{
						LinkList *Nodefind;
						int flags = IsNameUsed(head,msgstruct.infos);
						if(flags ==1){
                            ...
                            sprintf(buff,"%s dont want to receive file [%s]\n",nodeclient->nickname,nodeclient->filename);
                            //strcat(buff,"\n");
...
void echo_server(int sockfd, char * port) {
    ...
    memset(head->filename,'\0',MSG_LEN);
		ShowLink(head);	
		SalonList * salonhead;
	...
	while (1) {
	 ...
	 for(i = 0;i<nfds;i++){
		 if (pollfds[i].fd == sockfd && pollfds[i].revents & POLLIN) {
                ...
                memset(node->filename,'\0',MSG_LEN);
			 ...
			 rec1 = recv(pollfds[i].fd,&msgstruct, sizeof(struct message), 0);
				...
				if(rec1 == 0){
                    ...
                    if(msgstruct.type == FILE_REQUEST){
                        filesend(msgstruct,buff,pollfds[i],head,nodeclient);
                    }
					if(msgstruct.type == FILE_ACCEPT){
						fileaccept(msgstruct,buff,pollfds[i],head,nodeclient);
					}
					if(msgstruct.type == FILE_REJECT){
						filereject(msgstruct,buff,pollfds[i],head,nodeclient);
					}
				}// else		 
		 } // else if*/
	 } // for
